...
function testTemplateContent() {
  ...
  templates.forEach((template) => {
    ...
    if (fs.existsSync(templatePath)) {
      ...
      assert(
        !hasIncorrectPath,
        `${template} does not reference incorrect .claude/agents/ path`,
      );
    }
  });
}
...
`,
  ...
  try {
    ...
    assert(
      savedContent.includes('test-react-specialist'),
      'Agent content contains correct name',
    );
    assert(savedContent.includes('---'), 'Agent has YAML frontmatter');
    assert(
      savedContent.includes('color: blue'),
      'Agent has color specification',
    );
  } catch (error) {
    testResults.failed++;
  ...
  docsToCheck.forEach((docPath) => {
    if (fs.existsSync(docPath)) {
      ...
      if (content.includes('agents')) {
        assert(
          hasCorrectPath,
          `${filename} references correct .pk/agents/ path`,
        );
        assert(
          !hasIncorrectPath,
          `${filename} does not reference incorrect .claude/agents/ path`,
        );
      }
    }
  });
...
function runTests() {
  ...
  try {
    ...
    if (testResults.failed === 0) {
      log(
        '\n🎉 All tests passed! Agent creation functionality is working correctly.',
        'success',
      );
      process.exit(0);
    } else {
