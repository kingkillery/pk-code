...
export class ParallelTaskExecutor {
  ...
  async execute(): Promise<ParallelTaskResult[]> {
    ...
    while (taskQueue.length > 0 || runningTasks.length > 0) {
      ...
      while (
        runningTasks.length < this.maxConcurrency &&
        taskQueue.length > 0
      ) {
  ...
  private async executeTask(task: ParallelTaskConfig): Promise<void> {
    ...
    console.log(
      `📋 [${task.id}] Starting: "${task.prompt.substring(0, 50)}${task.prompt.length > 50 ? '...' : ''}"`,
    );
  ...
  private printSummary(results: ParallelTaskResult[]): void {
    ...
    if (failed > 0) {
      ...
      results
        ...
        .forEach((result) => {
          console.log(
            `   [${result.id}] ${result.prompt.substring(0, 40)}${result.prompt.length > 40 ? '...' : ''}`,
          );
          console.log(
...
export async function handleParallelCommand(argv: CliArgs): Promise<void> {
  ...
  if (prompts.length === 1) {
    console.log(
      '⚠️  Only one prompt provided. Consider using regular -p flag instead.',
    );
  }
