...
import type { TaskStatus } from './TaskPlanner.js';
...
export interface BlackboardQuery {
  ...
  notes?: {
    author?: string[];
    category?: Array<
      'info' | 'warning' | 'error' | 'question' | 'suggestion' | 'decision'
    >;
...
export class Blackboard {
  ...
  resolveBlockingIssue(
    ...
    const entry = this.taskStatuses.get(taskId);
    if (!entry || issueIndex < 0 || issueIndex >= entry.blockingIssues.length)
      return;
  ...
  getArtifactsByTask(taskId: string): Artifact[] {
    return Array.from(this.artifacts.values()).filter(
      (artifact) => artifact.createdBy === taskId,
    );
  }
   ...
   */
  createNote(
    note: Omit<SharedNote, 'id' | 'createdAt' | 'readBy' | 'acknowledgments'>,
  ): string {
    const id = this.generateId();
    const now = new Date();
  ...
  acknowledgeNote(noteId: string, agent: string, response?: string): boolean {
    ...
    note.acknowledgments = note.acknowledgments.filter(
      (ack) => ack.agent !== agent,
    );
  ...
  getNotesForAgent(agent: string, includeUnread?: boolean): SharedNote[] {
    return Array.from(this.notes.values()).filter((note) => {
      ...
      const isTargeted =
        note.targetAgents.length === 0 || note.targetAgents.includes(agent);
  ...
  search(query: BlackboardQuery): BlackboardSearchResults {
    ...
    // Search artifacts
    if (query.artifacts) {
      results.artifacts = Array.from(this.artifacts.values()).filter(
        (artifact) => {
          const q = query.artifacts!;

          if (q.type && !q.type.includes(artifact.type)) return false;
          if (q.tags && !q.tags.some((tag) => artifact.tags.includes(tag)))
            return false;
          if (q.createdBy && !q.createdBy.includes(artifact.createdBy))
            return false;
          if (q.createdAfter && artifact.createdAt < q.createdAfter)
            return false;
          if (q.createdBefore && artifact.createdAt > q.createdBefore)
            return false;
    ...
    if (query.notes) {
      results.notes = Array.from(this.notes.values()).filter((note) => {
        ...
        if (
          q.targetAgents &&
          !q.targetAgents.some((agent) => note.targetAgents.includes(agent))
        )
          return false;
        if (q.unreadBy && note.readBy.includes(q.unreadBy)) return false;
        if (q.requiresAck !== undefined && note.requiresAck !== q.requiresAck)
          return false;
        if (
          q.relatedTasks &&
          !q.relatedTasks.some((task) => note.relatedTasks.includes(task))
        )
          return false;
    ...
    if (query.taskStatuses) {
      results.taskStatuses = Array.from(this.taskStatuses.values()).filter(
        (status) => {
          ...
          if (
            ...
            (!status.assignedAgent ||
              !q.assignedAgent.includes(status.assignedAgent))
          )
            return false;
          if (
            q.hasBlockingIssues !== undefined &&
            status.blockingIssues.length > 0 !== q.hasBlockingIssues
          )
            return false;
          if (q.progressMin !== undefined && status.progress < q.progressMin)
            return false;
          if (q.progressMax !== undefined && status.progress > q.progressMax)
            return false;
