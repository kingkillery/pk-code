...
class MockOrchestrator {
      ...
      pareto: [
        {
          ...
          reason: 'Main orchestration logic, 70% of execution flow.',
        },
        {
          path: 'tests/integration.test.ts',
          reason: 'End-to-end validation, catches 80% of bugs.',
        },
        {
          path: 'config/models.json',
          reason: 'Model selection affects all AI interactions.',
        },
      ],
    };
  ...
  async executeExecution() {
    ...
    const executionSteps = [
      {
        thought: 'Initialize orchestrator configuration',
        action: 'read_config',
        observation: 'Config loaded successfully',
      },
      {
        thought: 'Apply guardrails for tool validation',
        action: 'setup_guardrails',
        observation: 'Retry logic enabled',
      },
      {
        thought: 'Execute strategic plan step by step',
        action: 'run_tests',
  ...
  validateTransition(fromPhase: string, toPhase: string) {
    ...
    if (
      !validTransitions[fromPhase] ||
      !validTransitions[fromPhase].includes(toPhase)
    ) {
      return {
        valid: false,
...
describe('Orchestrator Phase Transitions', () => {
  ...
  describe('Phase Transition Rules', () => {
    ...
    it('should validate all transition rules', () => {
      expect(orchestrator.validateTransition('init', 'metadata').valid).toBe(
        true,
      );
      expect(orchestrator.validateTransition('metadata', 'pareto').valid).toBe(
        true,
      );
      expect(orchestrator.validateTransition('pareto', 'strategic').valid).toBe(
        true,
      );
      expect(
        orchestrator.validateTransition('strategic', 'execution').valid,
      ).toBe(true);

      // Invalid transitions
      expect(orchestrator.validateTransition('init', 'pareto').valid).toBe(
        false,
      );
      expect(orchestrator.validateTransition('pareto', 'execution').valid).toBe(
        false,
      );
      expect(orchestrator.validateTransition('execution', 'pareto').valid).toBe(
        false,
      );
    });
  });
  ...
  describe('Phase Output Validation', () => {
    it('should validate Pareto phase YAML output format', async () => {
      ...
      result.output.pareto.forEach(
        (item: { path: unknown; reason: unknown }) => {
  ...
  describe('Complete Phase Flow', () => {
    it('should execute complete orchestrator flow', async () => {
      ...
      // Phase 1
      const paretoResult = await orchestrator.executePareto(
        'Implement feature X',
      );
